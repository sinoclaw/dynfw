...
- ✗ **不支持"增量长大复用"**：同预算下 warmstart 扩宽（D64→D96）**劣于重头训练**（+0.165，3 seed 稳；机理=warmstart 增益不随训练复利，预算机会成本更重）。**往更大规模扩 = 重头训练**（[`docs/GROWTH_INHERITANCE_REPORT.md`](docs/GROWTH_INHERITANCE_REPORT.md)）
