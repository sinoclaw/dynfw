...
## 4.2e v6.7 = v6.6 门控段换 FLA chunked 内核 —— 对 v6/v6.6 能力速度双料第一（对 TF 仅能力第一，速度/显存均输；见 §4.2g）（2026-09-13）
...
## 4.2g TF / v6.6 / v6.7 三方完整对比（2026-09-13 补测）—— 纠正"v6.7 对 TF 也全面领先"的误读

### 为什么要补这一节
§4.2e 的战绩表里 **tf 的「纯计算」与「显存」两栏是 `—`（当时没测）**。此前口头汇报中
曾用「实验 wall 536.1s vs tf 561.9s」充当速度证据，由此产生了「v6.7 对 TF 速度也赢」的印象。
**该 wall 正是本节和 §4.2c 反复声明不可用的口径**（含 ~537s mmap IO，三方同源，差异被糊平）。
本节补齐缺的两栏，纠正该误读。

### 补测数据（同口径：T=8192 / W=64 / batch=1 / 同代理 head / 预热后取中位 / 隔离 mmap IO）
| 架构 | 参数量 | 前向 | **fwd+bwd** | +optim | **peak 显存** |
|---|---|---|---|---|---|
| **v6.7**（FLA, bf16） | 45,188,098 | 19.5ms | **69.0ms** | 70.4ms | **5.13GiB** |
| **v6.6**（手写顺序循环） | 45,188,098 | 101.6ms | **1013.4ms** | 1003.2ms | 5.56GiB |
| **TF**（SDPA） | 39,444,352 | **13.1ms** | **45.4ms** | 46.5ms | **0.95GiB** |

| 相对 TF 的倍数（>1 = 比 TF 慢/费） | 前向 | fwd+bwd | peak 显存 |
|---|---|---|---|
| v6.6 | 7.77× | 22.33× | 5.84× |
| **v6.7** | **1.49×** | **1.52×** | **5.38×** |

| 相对 v6.6（v6.7 的改进） | 前向 | fwd+bwd | peak 显存 |
|---|---|---|---|
| v6.7 | 0.19×（快 5.2×） | **0.07×（快 14.7×）** | 0.92×（省 8%） |

### 能力（同 T=8192/1000step/3seed）
| 架构 | 参数量 | final_loss 中位 | std |
|---|---|---|---|
| **v6.7** | 45,188,098 | **5738.0** | 21.3 |
| v6.6 | 45,188,098 | 7142.6 | 80.4 |
| TF | 39,444,352 | 19734.9 | 43.6 |
...
| 速度（纯计算） | **TF** | v6.7 | v6.6 |
| 显存 | **TF** | v6.7 | v6.6 |
| 参数效率 | **TF**（参数最少） | v6.7 = v6.6 | — |

**结论（不得再含糊）**：
1. **v6.7 对 v6 / v6.6 是真·四项全赢**（同参数 45,188,098；能力 +19.7%、速度 +14.7×、显存 −8%）
2. **v6.7 对 TF 只赢能力（+70.9%），速度输 1.52×、显存输 5.38×，且 TF 参数还少 14.6%**
3. **绝不能用实验 wall 声称"速度快"** —— 那是 IO 糊账（三次复现：536/537/547s）
4. 三方**均未收敛**（末 200 step 仍降 24.5~26%）⇒ 以上能力排名是**趋势不是终局**

### 速度构成拆解（torch.profiler，CUDA 算子聚合）
```
TF   47.1ms = flash attn 43.1ms(91.6%) + 其余 ~4ms
  └ fmha 反向 30.6ms / 前向 12.5ms
v6.7 69.9ms = FLA kernel 22.6ms(32%) + elementwise 35.3ms(50%) + 其余 12.0ms
  └ fused_chunk_bwd 19.4ms / fwd 3.1ms；elementwise 系 4 项 21.0ms + 3 项 5.1ms + 3 项 5.5ms + reduce 3.7ms
```
**关键发现**：
- **v6.7 的核心算子（FLA 双向 22.6ms）比 TF 的 flash attention（43.1ms）快近一倍** ——
  同样做"长程聚合"，v6.7 的 kernel 更省
- **v6.7 慢的 100% 是 elementwise 拖累**（35.3ms 全在非矩阵运算：RoPE、logsigmoid 门控、
  permute/contiguous 搬运、LN、act）。根因是这些运算跑在 **67M 元素的宽张量 `[B,nh,T,N]`** 上，
  而 TF 跑在 **1M 元素的 `[B,T,D]`** 上（差 64×）
- **⇒ v6.7 的速度问题是"实现没优化干净"，不是"架构注定慢"**：优化路径明确
  （融合 RoPE 进门控 kernel、消除 permute+contiguous、合并 elementwise），**且不需要碰数学**

### 显存差额根因（同源问题）
| 阶段 | v6.7 | TF | 倍数 |
|---|---|---|---|
| 权重 | 0.170 GiB | 0.171 GiB | 1.0×（相同） |
| **前向峰值**（no_grad） | **1.971 GiB** | **0.217 GiB** | **9.1×** |
| fwd+bwd 峰值 | 5.13 GiB | 0.95 GiB | 5.38× |
