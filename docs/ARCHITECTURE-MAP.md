...
### 4.2c 速度账：raw 语义 + 融合实现（2026-09-12，公平桌）
**口径**（与 09-11 方案A 完全一致，保持可比）：`VOCAB=50257 D=256 NH=8 NL=6 W=256 B=2`，
autocast bf16，**训练 step（前向+反向+AdamW）**，warmup3/rounds3/iters3 取中位；**双方同优化等级（都 compile）**。
实现：`FWAttentionOpt5Raw`（手写 bmm + 并行 exclusive 前缀和 + tril(-1) 掩码缓存，**无 softmax**）。

| T | `TF+comp` | `v6raw-o5+comp` | 加速比 | 判据（跑前锁死） |
|---|---|---|---|---|
| 1,024 | 7.65 ms | 8.77 ms | 0.87× | ⚠️ 略输 |
| 8,192 | 37.64 ms | 39.76 ms | 0.95× | ⚠️ 打平 |
| 16,384 | 94.18 ms | 75.96 ms | **1.24×** | ⚠️ 已反超 |
| 32,768 | 274.27 ms | **150.25 ms** | **1.83×** | ✅ **卖点成立（≥1.5×）** |

**结论（诚实报）**：
1. **O(T) 卖点在长上下文兑现**：T=32K 快 **1.83×**（436k vs 239k tok/s）；**交叉点 ≈ T=10~12K**。
2. T≤8K 仍略输 5~13% —— 短 T 由常数因子主导，这是 O(T) vs O(T²) 的正常表现，不必粉饰。
3. `raw` vs `softmax` 在速度上**无差异**（1.83× vs 1.80×、1.24× vs 1.23×）⇒ **raw 是纯赚**（能力 +8.5 分、速度不亏）。
4. **compile 是命门**：eager 版 32K 时仅 0.75×（365 ms vs 150 ms），不 compile 一切优化不成立。
5. **修正旧账**：「v6 慢 TF 3.2×」= 未优化实现 vs SDPA 融合核的**脏数**（口径不对等），已作废。
6. 与 09-11 方案A 复现良好（T16K 1.23×→1.24×、T32K 1.80×→1.83×）⇒ 测量稳定可信。

**J1 验收（先过才测速）**：参数逐位相同（45,187,072）、logits maxdiff ≤ 8.35e-07（噪声级）、因果探针 maxdiff=0。
