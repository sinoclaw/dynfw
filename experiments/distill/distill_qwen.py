...
def make_student(args, teacher_vocab, device):
    ...
    if args.arch == 'fusedfw':
        ...
        m = BDHBlockFLALM(D=args.dim, nh=args.nh, vocab=teacher_vocab,
                          ...
                          share_qk=getattr(args, 'share_qk', False))
...
def main():
    ...
    ap.add_argument('--share-qk', action='store_true', dest='share_qk',
                    help='v6.7 的 q_f/k_f 复用同一张张量（K is Q，FLA kernel 只读）'
                         '；省一次布局搬运+bf16 转换')
    ...
    result = {
        ...
        'share_qk': bool(getattr(args, 'share_qk', False)),   # 口径标注：q/k 是否复用
